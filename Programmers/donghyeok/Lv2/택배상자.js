...
function solution(order) {
  var answer = 0; // 트럭에 실을 수 있는 상자 수
  const assistBelt = []; // 보조 컨테이너
  let box = 1; // 현재 메인 컨테이너에 있는 상자 번호

  for (const target of order) {
    while (box <= target) assistBelt.push(box++); // assistBelt [1,2,3,4]

    if (assistBelt[assistBelt.length - 1] === target) {
      assistBelt.pop(); // 마지막 상자 꺼내기
