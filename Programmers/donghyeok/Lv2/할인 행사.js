 ...
 * XYZ 마트에서 14일간 회원을 대상으로 할인하는 제품이 날짜 순서대로 치킨, 사과, 사과, 바나나, 쌀, 사과,
 * 돼지고기, 바나나, 돼지고기, 쌀, 냄비, 바나나, 사과, 바나나인 경우에 대해 알아봅시다.
...
function solution(want, number, discount) {
  // 원하는 제품 수량 Map 객체에 저장
  const wantMap = new Map();
  want.forEach((item, idx) => wantMap.set(item, number[idx]));
  ...
  for (let i = 0; i <= discount.length - 10; i++) {
    ...
    // 10일 동안 할인 제품 수량 Map 객체에 저장
    for (let j = i; j < i + 10; j++)
      countMap.set(discount[j], (countMap.get(discount[j]) || 0) + 1);

    // 원하는 제품 수량 Map 객체와 비교하여 일치하는지 확인
    let isValid = true;
